...
The results with IMU were worse. But to run it just uncomment `robot_description` from `my_robot.launch` and change `TRAJECTORY_BUILDER_2D.use_imu_data = false` to `true`.  
