...
Create `./imu_lidar_processor.py` with:
...
def put_together(timestamps, quats, seq, points):
    return f"{timestamps},{quats.x},{quats.y},{quats.z},{quats.w},{seq},{points.x},{points.y},{points.z}\n"
...
if __name__ == '__main__':
    # Initialize the node
    rospy.init_node('data_joiner_node')

    # Define the publisher
    pub = rospy.Publisher('joined_topic', String, queue_size=10)
...
...
ubuntu@ubiquityrobot:~$ ./imu_lidar_processor.py &
[INFO] [1731695040.521639]: PointCloud2 and IMU processor node started.
```

From a remote ROS create this `/joined_topic` subscriber, name it `./imu_lidar_subscriber.py`, to save data for posterior analisys:
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("/joined_topic", String, callback)
    rospy.spin()
```
