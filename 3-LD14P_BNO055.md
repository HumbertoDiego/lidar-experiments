...
```shell
ub20@ub20-VM:~$ nano catkin_ws/install_isolated/share/cartographer_ros/launch/my_robot.launch
```
...
This configuration expects a `bag_filename` argument to replay data from a recording (check playbag node) and also spawns a rviz window configured to visualize Cartographer's state (check rviz node). The cartographer_node points to my_robot.lua we created before and remaps imu topic to match 


roslaunch cartographer_ros my_robot.launch bag_filename:=/home/ub20/min.bag
