...
ub20@ub20-VM:~$ sudo sh -c 'echo "deb http://packages.ros.org/ros/ubuntu $(lsb_release -sc) main" > /etc/apt/sources.list.d/ros-latest.list'
ub20@ub20-VM:~$ sudo apt install -y curl
ub20@ub20-VM:~$ curl -s https://raw.githubusercontent.com/ros/rosdistro/master/ros.asc | sudo apt-key add -
ub20@ub20-VM:~$ sudo apt update
ub20@ub20-VM:~$ sudo apt install -y ros-noetic-desktop-full
ub20@ub20-VM:~$ source /opt/ros/noetic/setup.bash
ub20@ub20-VM:~$ echo "source /opt/ros/noetic/setup.bash" >> ~/.bashrc
ub20@ub20-VM:~$ env | grep ROS
ROS_VERSION=1
ROS_PYTHON_VERSION=3
ROS_PACKAGE_PATH=/opt/ros/noetic/share
...
ub20@ub20-VM:~$ sudo apt-get install -y python3-wstool python3-rosdep ninja-build stow
ub20@ub20-VM:~$ mkdir -p ~/catkin_ws/src
ub20@ub20-VM:~$ cd ~/catkin_ws
ub20@ub20-VM:~/catkin_ws$ wstool init src
ub20@ub20-VM:~/catkin_ws$ wstool merge -t src https://raw.githubusercontent.com/cartographer-project/cartographer_ros/master/cartographer_ros.rosinstall
ub20@ub20-VM:~/catkin_ws$ wstool update -t src
ub20@ub20-VM:~/catkin_ws$ sudo rosdep init || echo "rosdep already initialized"
ub20@ub20-VM:~/catkin_ws$ rosdep update
ub20@ub20-VM:~/catkin_ws$ rosdep install -r --from-paths src --ignore-src --rosdistro=${ROS_DISTRO} -y
ub20@ub20-VM:~/catkin_ws$ src/cartographer/scripts/install_abseil.sh
ub20@ub20-VM:~/catkin_ws$ catkin_make_isolated --install --use-ninja
ub20@ub20-VM:~/catkin_ws$ source ~/catkin_ws/install_isolated/setup.bash
ub20@ub20-VM:~/catkin_ws$ echo "source ~/catkin_ws/install_isolated/setup.bash" >> ~/.bashrc
```
...
Next connect to the Raspberry Access point (our ROS MASTER), ssh into ubiquity OS, record and retrieve a rosbag file. First step is validate our bagfile:
...
The IMU related errors can be corrected by recording just one `sensor_msgs/Imu` and include the one with gravity `/imu/raw`. The Laser error not sequential can be corrected by recording just `\scan`. 

Next, make a `.lua` configuration file:
...
ub20@ub20-VM:~$ nano catkin_ws/install_isolated/share/cartographer_ros/configuration_files/my_robot.lua
```

Edit as follows to match our setup:
...
Create `.launch` files for your SLAM scenario. The example files are in `catkin_ws/install_isolated/share/cartographer_ros/launch/`.

```shell
ub20@ub20-VM:~$ 
cp cp catkin_ws/install_isolated/share/cartographer_ros/launch/backpack_2d.launch catkin_ws/install_isolated/share/cartographer_ros/launch/myrobot.launch

ub20@ub20-VM:~$ nano catkin_ws/install_isolated/share/cartographer_ros/launch/myrobot.launch
```
